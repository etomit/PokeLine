...
@section('content')
<section class="union-room online-center-room">
    <header class="union-room-head">
        <a href="{{ route('home') }}">◀ {{ __('ui.back_home') }}</a>
        <div>
            <p class="eyebrow">GLOBAL TERMINAL // ONLINE</p>
            <h1>{{ __('ui.online_center') }}</h1>
            <nav class="online-center-tools" aria-label="{{ __('ui.online_center') }}">
                <a href="{{ route('teams.index') }}">⚙ {{ __('ui.manage_teams') }}</a>
                <a href="{{ route('teams.index') }}#online-inventory">▣ {{ __('ui.inventory') }}</a>
            </nav>
        </div>
    ...
    @if($teams->isEmpty())
        <section class="union-empty online-team-required">
            <div class="union-ball"></div>
            <div><h2>{{ __('ui.no_team') }}</h2><a class="pixel-button" href="{{ route('teams.index') }}">{{ __('ui.create_team') }}</a></div>
        </section>
    @else
        <div class="online-paths online-match-actions">
            <section class="online-path public-path">
                <div class="path-number">01</div>
                <div class="path-copy"><span class="online-chip"><i></i>{{ __('ui.players_online') }}</span><h2>{{ __('ui.quick_battle') }}</h2></div>
            ...
            <section class="online-path private-path">
                ...
                <div class="path-copy"><span class="online-chip private">⌁ {{ __('ui.private_room') }}</span><h2>{{ __('ui.invitation_battle') }}</h2></div>
                <div class="private-actions">
                    ...
                    <form action="{{ route('battle.online.join-code') }}" method="post">
                        ...
                        <label class="private-team-select">{{ __('ui.choose_team') }}<select name="team_id">@foreach($teams as $team)<option value="{{ $team->id }}">{{ $team->name }}</option>@endforeach</select></label>
    ...
    <div class="online-center-grid">
        <div class="online-center-column">
            @if($myBattles->isNotEmpty())
                <section class="online-trainers scroll-panel my-online-battles">
                    ...
                    <div class="online-battle-grid">
                        @foreach($myBattles as $currentBattle)
                            ...
                            <article class="online-battle-card {{ $currentBattle->status === 'active' ? 'is-live' : 'is-waiting' }}">
                                ...
                                @if($currentBattle->mode === 'online-private' && $currentBattle->status === 'waiting')<strong class="mini-room-code">{{ $currentBattle->code }}</strong>@endif
                                <div class="online-card-actions">
                                    ...
                                    @if($currentBattle->status === 'waiting' && $currentBattle->host_id === auth()->id())
                                        <form action="{{ route('battle.online.cancel', $currentBattle) }}" method="post">@csrf @method('DELETE')<button class="online-danger" type="submit">{{ __('ui.cancel_search') }}</button></form>
                                    @endif
                                </div>
                            </article>
                        @endforeach
            ...
            <section class="online-trainers scroll-panel available-trainers">
                <div class="online-trainers-head"><div><p class="eyebrow">LIVE SEARCH</p><h2>{{ __('ui.available_trainers') }}</h2></div><span>{{ $battles->count() }}</span></div>
                <div class="trainer-search-list">
                    @if($teams->isNotEmpty())
                        ...
                        @endforelse
                    @else
                        <div class="no-trainers"><p>{{ __('ui.no_team') }}</p></div>
                    @endif
                </div>
            </section>
        </div>
        ...
        <div class="online-center-column">
            <section class="online-trainers scroll-panel watch-center">
                <div class="online-trainers-head"><div><p class="eyebrow">BATTLE TV // LIVE</p><h2>{{ __('ui.watch_center') }}</h2></div><span>{{ $liveBattles->count() }} LIVE</span></div>
                <div class="online-battle-grid">
            ...
            <section class="online-trainers scroll-panel battle-history">
